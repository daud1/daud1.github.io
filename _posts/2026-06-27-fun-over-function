...
To draw the maze, I loop over the maze (2-dim numpy array) with an enumerator and draw the maze using spaces for the path and a character for the walls.
...
More improvements to the maze are probably needed for visual clarity like more spaces or ensuring the vertical walls are also completely sealed off but for now this should do. The second step and third steps should be easy enough to do with the `getch` and `addstr` methods.
